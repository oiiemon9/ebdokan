...
export default function ProductForm({ product }) {
    ...
    defaultValues: {
      ...
      images: product?.images
        ? product.images.map((imgUrl) => ({
            file: null, // Purono image er jonno file null thakbe
            url: typeof imgUrl === 'string' ? imgUrl : imgUrl.url, // URL check
            name: 'Existing Image',
          }))
        : [],
  ...
  const mutation = useMutation({
    mutationFn: (formData) =>
      apiFetch(isEditMode ? `/api/products/${product._id}` : '/api/products', {
        ...
        try {
          ...
          const uploadedImages = await Promise.all(
            normalizedData.images.map(async (img) => {
              // Jodi img.file na thake (mane eta database er purono image), tahole abar upload korar dorkar nei, direct URL return korbe
          ...
          if (isEditMode) {
            ...
            Object.keys(finalData).forEach((key) => {
              let oldValue = product[key];
              let newValue = finalData[key];
  ...
  return (
    <div className="min-h-screen ">
      <DashboardHeader
        ...
        description={
          isEditMode ? 'Update your product details' : 'Create a new product'
        }
      />
